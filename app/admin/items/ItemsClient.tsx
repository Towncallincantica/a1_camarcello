          ...
          <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', width: '100%', maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto', padding: '1.75rem' }}>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    style={{ ...s.btn('ghost'), marginBottom: '0.4rem' }}
                  >
                    {imagePreview ? 'Cambia immagine' : 'Carica immagine'}
                  </button>
                  {imagePreview && (
                    <button
                      onClick={() => { setImagePreview(null); setImageFile(null); setForm(f => ({ ...f, icon_url: '' })) }}
                      style={{ ...s.btn('danger'), marginLeft: '0.5rem' }}
                    >
                      Rimuovi
                    </button>
                  )}
                  <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.25)', marginTop: '0.3rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.25rem' }}>
              <div>
                ...
                <input type="text" value={form.claim_code} onChange={e => setForm(f => ({ ...f, claim_code: e.target.value.toUpperCase() }))} style={{ ...s.input, fontFamily: 'Space Mono, monospace' }} placeholder="CHIAVE42" />
              </div>
              <div>
                <label style={s.label}>Limite claim</label>
